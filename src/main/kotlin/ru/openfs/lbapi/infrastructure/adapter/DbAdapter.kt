    ...
    fun getVGroupsAndServices(agreementId: Long): ServiceInfo? {
        val sql = """
            SELECT
	            ...
	            COALESCE((SELECT tmd.rent FROM billing.tariff_modifiers tmd where tmd.vg_id = v.vg_id and tmd.tar_id = t.tar_id and tmd.service_id is null and tmd.rent is not null and tmd.timefrom <= CURRENT_DATE()),-1) as tarDiscount,
            ...
            .map { rows ->
                rows.groupBy { it.getLong("vg_id") }
                    .map { (k, rows) ->
                        ...
                        val tarRent = when {
                            dbAmount in 0.0..<dbRent -> dbAmount
                            else -> dbRent
                        }
